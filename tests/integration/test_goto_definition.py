...
mal_symbol_def_reaches_points = list(zip([
...
goto_input_location_and_files = itertools.chain(mal_find_symbols_in_scope_points,
...
find_symbols_in_scope_expected_results = list(zip(goto_input_location_and_files,
                                                  goto_expected_location_and_files))
...
def open_document_notification(
        ...
        client_messages: list[dict]) -> FixtureCallback[dict]:
...
def goto_definition_client_messages(
        ...
        definition_request: FixtureCallback[dict]) -> FixtureCallback[typing.BinaryIO]:
...
def parameter_id(argvalue: (((int, int), str), ((int, int), str))) -> object:
    ...
    return (f"{origin_file}:L{origin_line},C{origin_char}"
            "-"
